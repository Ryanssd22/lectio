...
parsedSeasons = {}
for date, season in liturgySeasons.items():
    parsedSeasons[date.strftime("%Y-%m-%d")] = season
...
with open(f"./liturgies/liturgy{year}.json", "w", encoding="utf-8") as f:
    ...
    print(json.dumps(parsedLiturgy))

with open(f"./liturgies/liturgy{year}season.json", "w", encoding="utf-8") as f:
    json.dump(parsedSeasons, f, indent=2, ensure_ascii=False)
